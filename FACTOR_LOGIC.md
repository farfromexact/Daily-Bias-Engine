...
- ETF 成交额是真实 Wind 数据，但它只是 ETF 净申购/份额变化的 proxy。
- 指数样本上涨比例是真实 Wind 指数价格派生结果，但它只是全市场上涨家数的 proxy。
- 海外指数高低价振幅是真实 Wind 价格派生结果，但它只是 VIX/CNH/美元压力的 proxy。
- `margin_balance_momentum` 当前还不是 Wind 真实两融字段，而是由 ETF 成交额派生的演示 proxy。
