...
FACTOR_LOGIC = [
    {
        ...
        "group": "股指期货结构",
        "data_source": "Wind 真实日频收盘价：IF.CFE 与 000300.SH。",
        ...
        "caveat": "数据是真实 Wind 数据；MVP 口径只用 IF 与沪深300，尚未展开 IF/IH/IC/IM 跨品种结构。",
    },
    {
        "factor_name": "futures_open_interest_momentum",
        "group": "股指期货结构",
        "data_source": "Wind 真实日频期货持仓量：IF.CFE oi。",
    ...
    },
    {
        ...
        "group": "利率与债券",
        "data_source": "Wind 真实利率/收益率序列：DR007.IB、CGB10Y.IB。",
    ...
    },
    {
        ...
        "group": "ETF 与资金流",
        "data_source": "Wind 真实 ETF 日频成交额：510300.SH、510500.SH。",
        "raw_formula": "ETF 成交额 5 日变化率",
        "normalization": "20日滚动 z-score；方向分 = clip(zscore, -2, 2) / 2",
        "direction": "ETF 成交额放大被视为资金参与增强，偏 Risk-On。",
        "caveat": "这里的 proxy 指因子口径：成交额是真实 Wind 数据，但它不等于真实 ETF 净申购/份额变化。",
    },
    {
        "factor_name": "margin_balance_momentum",
        "group": "ETF 与资金流",
        "data_source": "当前由真实 ETF 成交额派生演示字段；尚未接真实两融余额。",
        "raw_formula": "margin_balance 5 日变化率；若无真实字段则由成交额生成演示 proxy",
        "normalization": "20日滚动 z-score；方向分 = clip(zscore, -2, 2) / 2",
        "direction": "融资/杠杆资金回升偏 Risk-On，回落偏 Risk-Off。",
        "caveat": "这是当前最需要替换的 proxy：快照真实，但该字段不是 Wind 两融真实字段。",
    },
    {
        "factor_name": "overseas_market_momentum",
        "group": "海外隔夜",
        "data_source": "Wind 真实海外指数日频价格：SPX.GI、HSI.HI。",
    ...
    },
    {
        ...
        "group": "海外隔夜",
        "data_source": "Wind 真实海外指数高低价；振幅为由价格派生的波动 proxy。",
    ...
    },
    {
        ...
        "group": "A股市场结构",
        "data_source": "Wind 真实指数日频开收盘价：000300.SH、000905.SH、000852.SH。",
        "raw_formula": "样本指数中 close > open 的比例 - 0.5",
        "normalization": "20日滚动 z-score；方向分 = clip(zscore, -2, 2) / 2",
        "direction": "上涨扩散度提高偏 Risk-On，扩散度走弱偏 Risk-Off。",
        "caveat": "这里的 proxy 指用指数样本替代全市场宽度；后续应接上涨家数、涨跌停、行业扩散度。",
    },
    {
        "factor_name": "ashare_turnover_momentum",
        "group": "A股市场结构",
        "data_source": "Wind 真实指数日频成交量。",
